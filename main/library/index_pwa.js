...
const SETTING = {
  ...
  LocalStorage: {
    Login: {
      email: "pwa-signin-email",
      password: "pwa-signin-password",
      code: "pwa-signin-code",
      token: "pwa-signin-token",
    },
    Display: {
      theme: "pwa-display-theme",
      font: "pwa-display-font",
      language: "pwa-display-lang",
    },
  },
};
...
const Security = {
  OAuth: {
    ...
    getInfo: async () => {
      ...
      if (code) {
        return await fetch(SETTING.Server + "/oauth/pwa-google-code", {
          method: "POST",
          cache: "no-cache",
          headers: {
            "Content-Type": "application/json",
